#!/bin/bash
#SBATCH --image=deeplearnphysics/dune-nd-sim:ub20.04-cpubase-edep2supera
#SBATCH --qos=regular
#SBATCH --constraint=cpu
#SBATCH --nodes=1
#SBATCH --time=00:30:00
#SBATCH --account=dune
#SBATCH --array=0-999


NUM_EVENTS=100
CONFIG_GDML="config/nd_hall_only_lar.gdml"
EDEPS_DIR="/global/cfs/cdirs/dune/users/rradev"
OUTPUT_ROOT="${EDEPS_DIR}/edeps_out_${SLURM_ARRAY_TASK_ID}.root"
OUTPUT_H5="${EDEPS_DIR}/edeps_out_${SLURM_ARRAY_TASK_ID}.h5"
EDEP_MACRO="config/edep.mac"

# Run the edep-sim command
shifter edep-sim -e $NUM_EVENTS -g $CONFIG_GDML -o $OUTPUT_ROOT $EDEP_MACRO

# If edep_out.h5 exists locally, delete it
if [ -f $OUTPUT_H5 ]; then
    rm $OUTPUT_H5
fi

# Convert to h5
shifter python3 dump_tree.py $OUTPUT_ROOT $OUTPUT_H5
